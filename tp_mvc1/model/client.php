...
class client {
	...
	public function getInfosClient($leClient) {
		$liste = "SELECT * FROM client WHERE emailClient = :mail;";

		$req = $this->pdo->prepare($liste);
	...
	public function inscriptionClient($nom, $prenom, $email, $motDePasse, $rue, $cp, $ville, $tel) {
		$inserer = "INSERT INTO client (nomClient, prenomClient, emailClient, motDePasseClient, rueClient, cpClient, villeClient, telClient) VALUES (:nom, :prenom, :email, :motDePasse, :rue, :cp, :ville, :tel);";
		$motDePasse = password_hash($_POST['motdepasse'], PASSWORD_DEFAULT);
		$req = $this->pdo->prepare($inserer);
